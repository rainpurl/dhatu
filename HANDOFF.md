...
**B. Icons + privacy policy: DONE.** PNG icons (192/512 + apple-touch 180) were
generated from the logo with `qlmanage` + `sips` (no rsvg/inkscape/cairosvg on
this Mac). `public/manifest.webmanifest` holds name/theme/icons metadata and is
linked from `index.html`. **The user did NOT want a PWA**, so the service worker
and the TWA `assetlinks.json` were removed; only the icons + manifest metadata
remain (they are reused by the native app). A hostable **privacy policy** is at
...
- `capacitor.config.json` (appId `dev.dhatu.app`, `webDir: dist`).
...
- Owner steps (all in NATIVE.md): `npm install` -> `npx cap add android` ->
  register an **Android app in Firebase** (package `dev.dhatu.app`, add signing
...
client-ID URL scheme. The same `src/firebase.js` native path works. Android first.

Reminder: store distribution costs money (Apple $99/yr, Google $25 once), which
crosses the hard no-cost rule, so it is an explicit owner decision. The code and
builds stay free.
